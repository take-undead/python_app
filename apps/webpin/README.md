...
| 入力 | 実際の接続先 | 備考 |
| --- | --- | --- |
| `1.132` | `ws://192.168.1.132/ws` | `STATIC_IP`（既定） |
| `2.132` | `ws://192.168.2.132/ws` | ネットワークが 192.168.2.x のとき |
| `t-iot_mobile.local` | `ws://t-iot_mobile.local/ws` | `HOSTNAME` の mDNS 名 |

ローカルネットワーク前提なので、`1.132` のように **`192.168.` は省略できる**
（`ws://` と `/ws` も自動で補う）。`192.168.1.132` や `ws://192.168.1.132/ws` の
ようにフルで入れてもよい。接続が切れた場合は 3 秒ごとに自動で再接続する。
